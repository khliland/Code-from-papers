function [beta, W, P, T, PtW, Wb, order, count, C, F] = ROSAfixed(X, Y, A, combos, fix)
% -------------------------------------------------------------------------
...
for a = 1:A
    ...
    F(:,a) = sqrt(mean(r.^2))'; % Block-wise fit to residual response.
    ...
    if length(fix) >= a % Fixed selection up to given component
        if length(fix{a}) == 1
            mn = 0; mr = fix{a};
        else
            mr = 0;
            for j=1:size(comb,1)
                if all(ismember(fix{a}, unique(comb(j,:))))
                    mr = j;
                end
            end
            rrr(mr) = 0;
